...
2. If your server supports In-Band Registration, you can register from Emacs by typing =C-u M-x jabber-connect=.
   1. Enter your desired JID in the form =username@server.tld=.
   2. Fill out and send the registration form.

3. If your server does not support In-Band Registration, register on the server website.

** How to use jabber.el
:PROPERTIES:
:CUSTOM_ID: how-to-use
:END:
1. Connect to your server by typing =C-x C-j C-c= (=jabber-connect=). Enter your JID and password.

# basic operation - switching buffers, chatting, contacts

3. If you wish to disconnect, type =M-x jabber-disconnect= or =C-x C-j C-d=.
