...
3. Make your edits to =jabber.org= (not =jabber.el=), and save.

4. Tangle the file -
   #+BEGIN_SRC
   M-x compile RET make -Bk
   #+END_SRC
   This also byte compiles it. Address any byte compilation errors.

   You can also run =make dev= to tangle, compile, and run linters in one go, and address their output.

5. Document user-facing changes in the [[file:NEWS][NEWS]] file. Update other documentation in this [[file:README.org][README.org]] if necessary.

6. Push and create your PR, or send a patch using [[https://git-send-email.io/][git-send-email]] to [[mailto:wgreenhouse@tilde.club][wgreenhouse@tilde.club]]
