...
The file hexrgb.el (http://www.emacswiki.org/emacs/hexrgb.el) used to be required for the MUC nick coloring feature. However, the code depending on it has been reworked to instead use the built-in color.el library.
...
3. Make your edits to, then run =make dev= to tangle, compile, and run linters. Try to address their output.
