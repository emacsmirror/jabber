...
(defun jabber-httpupload-parse-slot-answer (xml-data)
  ...
  the slot for a file.
  The returned date are the PUT URL and the GET URL."
  (list
...
#+BEGIN_SRC emacs-lisp
(defun jabber-httpupload--request-slot-failed (_jc xml-data data)
...
CONTENT-TYPE is the MIME type of the file."
  (jabber-send-iq jc (cdr (jabber-httpupload-has-support jc)) "get"
                  ...
                  #'jabber-httpupload--request-slot-succesful (list filename)
                  #'jabber-httpupload--request-slot-failed (list filename)))
#+END_SRC

* TODO Uploading the file
Use the HTTP protocol to upload the file to the PUT URL provided by the slot.

** TODO jabber-httpupload-upload-file
#+BEGIN_SRC emacs-lisp
  (defun jabber-httpupload-upload-file (_filepath _size _content-type _put-url)
    "Update the given file at FILEPATH to the PUT-URL provided.
  The SIZE and CONTENT-TYPE (MIME type) of the file must match the one provided
