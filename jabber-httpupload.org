...
The returned list has the PUT URL and the GET URL."
  (list
   (jabber-xml-get-attribute (jabber-xml-path (car xml-data) '(slot put)) 'url)
   (jabber-xml-get-attribute (jabber-xml-path (car xml-data) '(slot get)) 'url)))
#+END_SRC

** jabber-httpupload--request-slot-succesful
#+BEGIN_SRC emacs-lisp
(defun jabber-httpupload--request-slot-succesful (jc xml-data data)
